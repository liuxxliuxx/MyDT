...
def main():
    ...
    parser.add_argument('--executions',nargs='+',default=['reference','optimized','compiled'],choices=['reference','optimized','compiled'])
    ...
    from emotion_ssm.train.staged_v34.data import QuerySampler
    ...
    if rank==0:
        fixture=Path(tempfile.mkdtemp(prefix='fixture_',dir=root))
        _,o,t,c,_,_,bank,settings=case(fixture)
        bank['domain']=torch.tensor([0,0,1,1,1,1,2,2])
        from emotion_ssm.train.staged_v34.semantics import class_audit
        bank['class_audit']=class_audit(bank,settings)
