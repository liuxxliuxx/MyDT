"""v3.3: query-balanced staged training with fixed and replayed deployment controls."""
import argparse
import copy
import json
import math
...
from .data import FeatureViews,build_bank,QuerySampler
from .missing import digest
from .optimization import PersistentOptimizers
from .objectives import loss,supports
from .baselines import fit,refit_output_calibration
from .evaluation import evaluate,deployment_gate
...
DEFAULTS=dict(OLD_DEFAULTS,missing_seed=88173,tbptt_events=32,gold_query_budget=64,
    ...
    semantic_absolute_tolerance=.02)
...
def run(request,execution_override=None,stop_after=None):
    ...
    if resume:
        if source['kind']!='dynamics_staged_v33' or source['config']['train'].get('staged_revision')!=REVISION:
            raise ValueError('Old optimizer semantics cannot resume v3.3; use explicit initialization')
    ...
    if settings['objective_mode'] not in ('vector_only','weighted_joint'):raise ValueError('Unknown objective mode')
    ...
    config['observer']=observer.construction();config['execution']=dict(mode=execution,revision='v33-global-query-counts')
    config['paths']['resume']=''
    ...
    def save(names,block,block_step,bank_models,train_bank,sampler,refresh_step):
        ...
        run_state=dict(world_size=size,initial_models=initial_models,initial_metrics=initial_metrics,
            locks=locks,cohort=cohort,best_vector=best_vector,best_deploy=best_deploy,
        ...
        if rank==0:
            for name in names:
                save_checkpoint(output/name,models,config,{**source['construction'],'observer':observer.construction(),'state':core.get_config()},
                    'dynamics_staged_v33',step=global_step,optimizer=optimizers,metrics=metrics,run_state=run_state)
    ...
    for block,(phase,length) in enumerate(stage_plan(settings)):
        ...
        if continuing:
            ...
            train_bank=bank(training,selected,f'origins_block{block}',protocol='train_mixed',expected=source['run_state']['bank_producer'])
            for name,model in models.items():model.load_state_dict(current[name])
        else:
            refresh_step=0;bank_models=cpu_models(models)
            train_bank=bank(training,selected,f'origins_block{block}',protocol='train_mixed')
        ...
        emit('phase_started',phase=phase,block=block,length=length,origins=len(train_bank['keys']),
            ...
            tbptt_protocol='full-dialogue-prefix-bank-plus-32-event-replay' if online else 'frozen-origins',
            optimizer_updates=optimizers.updates)
        seconds=[]
        for block_step in range(begin+1,length+1):
            if online and block_step>refresh_step+settings['update_refresh_steps']:
                bank_models=cpu_models(models);refresh_step=block_step-1
                previous_sampler=sampler.state_dict()
                train_bank=bank(training,selected,f'origins_block{block}',protocol='train_mixed')
                sampler=QuerySampler(train_bank,seed+block*101,chronological=True);sampler.load_state_dict(previous_sampler)
            ...
            if global_step%settings['validation_every']==0 or block_step==length:
                ...
                deploy_score=metrics['live']['methods']['learned']['macro_mse']
